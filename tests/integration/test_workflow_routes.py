...
async def test_workflow_routes(test_client: AsyncClient, db_pool: asyncpg.Pool):
    ...
    r = await test_client.post(
        ...
        json={"name": "Custom Projector", "category": "other"},
        headers=t_headers,
    )
    ...
    lvl_r = await test_client.post(
        "/api/v1/students/levels", json={"name": "Grade 11"}, headers=admin_headers
    )
    ...
    cls_r = await test_client.post(
        ...
        json={"name": "11A", "level_id": lvl_id, "head_teacher_id": t_uid},
    ...
    r = await test_client.post(
        f"/api/v1/events/{event_id}/audience", json={"class_ids": [class_id]}, headers=t_headers
    )
    assert r.status_code == 200
    ...
    r = await test_client.get(
        f"/api/v1/events/{event_id}/audience/prediction?class_ids={class_id}", headers=t_headers
    )
    assert r.status_code == 200
    ...
    assert float(pub_events[0]["class_mappings"][0]["ticket_price"]) == 0.0
