...
async def test_state_machine_workflow(db_pool: asyncpg.Pool, monkeypatch):
    ...
    await TenantService.add_resources_to_event(
        ...
        event_id=event_id,
        resources_list=[
            {"resource_type_id": bus_type["id"], "description": "Transport", "quantity": 2}
        ],
    ...
    await TenantService.set_resource_cost(
        tenant_id="tenant_a",
        resource_id=(await TenantService.get_resource_summary("tenant_a", event_id))["resources"][
            0
        ]["id"],
        unit_price=100.0,
        currency="JOD",
    ...
    with pytest.raises(ValueError):
        await TenantService.transition_event(
            "tenant_a", event_id, "manager_reject", manager, reason=""
        )

    event = await TenantService.transition_event(
        "tenant_a", event_id, "manager_reject", manager, reason="Needs more details"
    )
    assert event["status"] == "draft"
