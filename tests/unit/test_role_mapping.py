...
class TestRoleMapping:
    def test_current_user_direct_role(self):
        user = CurrentUser(
            user_id="user_123", tenant_id="tenant_a", role="teacher", roles=["teacher"]
        )
    ...
    def test_current_user_super_admin_bypass(self):
        user1 = CurrentUser(
            user_id="user_123", tenant_id="tenant_a", role="super_admin", roles=["super_admin"]
        )
    ...
    def test_has_intersection_capabilities(self):
        assert (
            authz.require(user_role=["school_admin"], allowed_roles={"school_admin", "super_admin"})
            is True
        )

        assert authz.require(user_role=["teacher"], allowed_roles={"school_admin"}) is False

        assert authz.require(user_role=["super_admin"], allowed_roles={"school_admin"}) is True

    def test_school_admin_event_delete_permission(self):
        admin_user = CurrentUser(
            user_id="admin_123", tenant_id="tenant_a", role="school_admin", roles=["school_admin"]
        )
        assert admin_user.has_role("school_admin") is True
        assert (
            admin_user.has_any_role(
    ...
    def test_admin_role_alias_mapping(self):
        admin_alias_user = CurrentUser(
            user_id="admin_456", tenant_id="tenant_a", role="admin", roles=["admin"]
        )
        ...
        assert (
            authz.require(
                user_role=["admin"], allowed_roles={"school_admin", "super_admin", "admin"}
            )
            is True
        )
